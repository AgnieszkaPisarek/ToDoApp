...
describe('Creating and deleting tasks', () => {
  ...
  it('should successfully create many tasks and then delete them', () => {
    ...
    cy.get('.thingToDo').eq(numberOfTasks).should('have.value', 'Wash the dishes')
    cy.get('.completedText').should('contain.text', 'Completed ' + numberOfCompletedTasks + ' of ' + ++numberOfTasks)

    cy.get('.createTaskInput').type('Take out the trash')
    cy.get('.addTaskButton').first().click()
    cy.get('.thingToDo').eq(numberOfTasks).should('have.value', 'Take out the trash')
    cy.get('.completedText').should('contain.text', 'Completed ' + numberOfCompletedTasks + ' of ' + ++numberOfTasks)

    cy.get('.createTaskInput').type('Take the dog for a walk')
    cy.get('.addTaskButton').first().click()
    cy.get('.thingToDo').eq(numberOfTasks).should('have.value', 'Take the dog for a walk')
    cy.get('.completedText').should('contain.text', 'Completed ' + numberOfCompletedTasks + ' of ' + ++numberOfTasks)

    cy.get('.createTaskInput').type('Do the homework')
    cy.get('.addTaskButton').first().click()
    cy.get('.thingToDo').eq(numberOfTasks).should('have.value', 'Do the homework')
    cy.get('.completedText').should('contain.text', 'Completed ' + numberOfCompletedTasks + ' of ' + ++numberOfTasks)
...
describe('Editing tasks', () => {
  ...
  it('should visibly edit task', () => {
    ...
    cy.get('.task').should('exist').then(() => {
      cy.get('.thingToDo').eq(0).invoke('val').then((previousTaskDescription) => {
        ...
        cy.get('input.thingToDo').eq(0).clear().type(String(previousTaskDescription))
      })

      cy.get('.date').eq(0).invoke('val').then((previousDate) => {
        cy.get('.date').eq(0).clear()
        cy.get('.date').eq(0).type('2012-12-12')
        cy.get('.date').eq(0).should('have.value', '2012-12-12')
        cy.get('.date').eq(0).clear().type(String(previousDate))
      })
    })
  })
